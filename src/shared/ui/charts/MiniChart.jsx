...
export default function MiniChart({
  ...
  if (!pts)
    return (
      <div className="dr-mini-empty" style={{ width: width, height: height }}>
        No data
      </div>
    );
  const allVals = []
      ...
      {showAxes &&
        (() => {
          ...
          for (let xi = 0; xi <= xTicks; xi++) {
            ...
            const label =
              avg[idx] && avg[idx].t
                ? new Date(avg[idx].t).toISOString().slice(0, 10).slice(5)
                : "";
