...
export default function RechartsAnalytics({ series, width = 680, height = 260, accent = '#0ea5a6', primary = '#2563eb' }) {
  ...
  // compute data bounds
  const dataXs = pts.map(p => p.x);
  const dataMin = dataXs.length ? Math.min(...dataXs) : (new Date()).getTime();
  const dataMax = dataXs.length ? Math.max(...dataXs) : (new Date()).getTime();

  // months array already built above (last 12 months with tLabel)
