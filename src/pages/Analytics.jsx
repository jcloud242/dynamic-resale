...
import AnalyticsChart from '../shared/AnalyticsChart.jsx';
...
function clientComputeEstimateFromActives(listings = [], opts = {}) {
  const prices = (listings || []).map(l => {
    try { return Number(l.avgPrice || l.price || (l.price && l.price.value) || null); } catch (e) { return null; }
  }).filter(n => Number.isFinite(n));
  const sampleSize = prices.length;
  if (!sampleSize) return { sampleSize: 0, confidence: 'low', scenarios: {}, base: null };
  ...
  const defaults = { binFactor: 0.88, feeRate: 0.15, shippingEstimate: 3.5, refurbCost: 0 };
  ...
  return { sampleSize, cleanedCount: cleaned.length, avgActive: avgActive !== null ? Number(avgActive.toFixed(2)) : null, medianActive: median !== null ? Number(median.toFixed(2)) : null, confidence, scenarios, generatedAt: new Date().toISOString() };
}
...
function buildAggregateSeries(recent) {
  ...
  for (const it of (recent || [])) {
    ...
    for (const p of it.timeSeries.avg) {
      if (!p || typeof p.t === 'undefined' || typeof p.v === 'undefined') continue;
  ...
  for (const p of all) {
    ...
    map[d].sum += p.v; map[d].count += 1;
  }
  const keys = Object.keys(map).sort();
  const avg = keys.map(k => ({ t: k, v: Math.round((map[k].sum / map[k].count) * 100) / 100 }));
  return { avg, min: [], max: [] };
}
    ...
    if (this.state.error) {
      return (
        <div className="dr-analytics" style={{padding:20}}>
          ...
          <pre style={{whiteSpace:'pre-wrap'}}>{String(this.state.error && this.state.error.message ? this.state.error.message : this.state.error)}</pre>
        </div>
      );
    }
...
function AnalyticsInner({ item, onBack }) {
  ...
  const recent = React.useMemo(() => {
    try { return JSON.parse(localStorage.getItem('dr_recent') || '[]') || []; } catch (e) { return []; }
  }, []);
  const [selectedItem, setSelectedItem] = useState(null);
  // slider controls target profit percentage (profit margin)
  const [profitPct, setProfitPct] = useState(30);
  const [buyPrice, setBuyPrice] = useState(() => {
    try { return Number(localStorage.getItem('dr_last_buy_price') || 0); } catch (e) { return 0; }
  });
  ...
  function makeYearSeries(base = 30) {
    ...
    return { avg: arr, min: arr.map(p => ({ t: p.t, v: Math.max(0, p.v - 8) })), max: arr.map(p => ({ t: p.t, v: p.v + 8 })) };
  }
  ...
  const series = React.useMemo(() => {
    return itemToShow ? (itemToShow.timeSeries || makeYearSeries(itemToShow.avgPrice || 30)) : makeYearSeries(30);
  ...
  function scoreItem(a, bTokens) {
    ...
    let score = 0;
    for (const t of aTokens) if (bTokens.includes(t) && t.length > 2) score += 1;
    // reward same platform
    if ((a.platform || a.category || '') && itemToShow && (itemToShow.platform || itemToShow.category)) {
      if ((a.platform || a.category || '').toLowerCase() === (itemToShow.platform || itemToShow.category || '').toLowerCase()) score += 1;
    }
    return score;
  }
  ...
  top5 = top5.map(r => ({ title: r.title || r.query, platform: r.platform || r.category || '', year: (r.year || '').toString().slice(0,4), price: r.avgPrice || r.maxPrice || r.minPrice || 0, date: (r.fetchedAt ? new Date(r.fetchedAt).toISOString().slice(0,10) : '') }));

  // rolling average (simple mean over recent with avgPrice)
  const avgPrices = (recent || []).map(r => r && typeof r.avgPrice === 'number' ? r.avgPrice : null).filter(Boolean);
  const rollingAvg = avgPrices.length ? Math.round((avgPrices.reduce((a,b)=>a+b,0)/avgPrices.length)*100)/100 : null;

  // estimate days to sell: crude estimate using soldListings per month if available
  const soldPerMonth = (itemToShow && Array.isArray(itemToShow.soldListings)) ? Math.max(0.1, itemToShow.soldListings.length / 6) : 0.5; // placeholder
  ...
  const baseScenario = estimator && estimator.scenarios ? estimator.scenarios.base : null;
  const optimistic = estimator && estimator.scenarios ? estimator.scenarios.optimistic : null;
  const conservative = estimator && estimator.scenarios ? estimator.scenarios.conservative : null;
  ...
  const categoryKey = (itemToShow && (itemToShow.category || itemToShow.platform || '')).toString().toLowerCase();
  const categoryCfg = categoryFeeMap[categoryKey] || null;
  const feeRate = (categoryCfg && typeof categoryCfg.feeRate === 'number') ? categoryCfg.feeRate : ((itemToShow && typeof itemToShow.feeRate === 'number') ? itemToShow.feeRate : (serverEstimate && typeof serverEstimate.feeRate === 'number' ? serverEstimate.feeRate : defaultFeeRate));
  const shippingEstimate = (categoryCfg && typeof categoryCfg.shippingEstimate === 'number') ? categoryCfg.shippingEstimate : ((itemToShow && typeof itemToShow.shippingEstimate === 'number') ? itemToShow.shippingEstimate : (serverEstimate && typeof serverEstimate.shippingEstimate === 'number' ? serverEstimate.shippingEstimate : defaultShipping));
  // Determine the sell price and net expected (prefer server estimate if present)
  const sellPrice = (serverEstimate && serverEstimate.scenarios && serverEstimate.scenarios.base && typeof serverEstimate.scenarios.base.expectedSale === 'number') ? serverEstimate.scenarios.base.expectedSale : (baseScenario && baseScenario.expectedSale ? baseScenario.expectedSale : (itemToShow && itemToShow.avgPrice ? itemToShow.avgPrice : null));
  const netAfterFees = (typeof sellPrice === 'number') ? Number((sellPrice * (1 - feeRate) - shippingEstimate).toFixed(2)) : null;
  const feeAmount = (typeof sellPrice === 'number') ? Number((sellPrice * feeRate).toFixed(2)) : null;

  // determine the net expected value (after fees & shipping) to base suggested buy calculations on
  const netFromEstimator = (serverEstimate && serverEstimate.scenarios && serverEstimate.scenarios.base && typeof serverEstimate.scenarios.base.netExpected === 'number') ? serverEstimate.scenarios.base.netExpected : (baseScenario && baseScenario.netExpected ? baseScenario.netExpected : null);
  const netForSuggested = netFromEstimator !== null ? netFromEstimator : netAfterFees;
  ...
  const suggestedBuy = (typeof sellPrice === 'number') ? (function() {
    const totalRevenue = sellPrice + shippingPaid;
    const val = totalRevenue * (1 - feeRate - targetMargin) - shippingEstimate;
  ...
  const authoritativeBuy = (itemToShow && typeof itemToShow.bought === 'number') ? itemToShow.bought : (buyPrice && Number(buyPrice) > 0 ? Number(buyPrice) : null);
  ...
  const requiredSalePrice = (authoritativeBuy !== null) ? (function() {
    const denom = (1 - feeRate) - targetMargin;
    if (!(denom > 0)) return null; // impossible to reach such a high margin with these fees
    const totalRevenueNeeded = (authoritativeBuy + shippingEstimate) / denom;
    const saleNeeded = totalRevenueNeeded - shippingPaid;
    return Number(saleNeeded.toFixed(2));
  })() : null;
  ...
  const displayedSalePrice = (function() {
    if (authoritativeBuy !== null && requiredSalePrice !== null) {
      ...
      if (marketSell !== null && marketSell >= requiredSalePrice) return marketSell;
  ...
  const profitWhenBought = (authoritativeBuy !== null && displayedSalePrice !== null) ? (function() {
  ...
  const profitWhenSuggested = (suggestedBuy !== null && displayedSalePrice !== null) ? (function() {
    const totalRevenue = displayedSalePrice + shippingPaid;
    const fees = totalRevenue * feeRate;
  ...
  const displayProfitWhenBought = (profitWhenBought !== null) ? Math.max(0, profitWhenBought) : null;
  const displayProfitWhenSuggested = (profitWhenSuggested !== null) ? Math.max(0, profitWhenSuggested) : null;

  // profit percent (relative to total revenue = sale + shippingPaid) for display
  const profitPctWhenBought = (displayProfitWhenBought !== null && displayedSalePrice !== null) ? Number(((displayProfitWhenBought / (displayedSalePrice + shippingPaid)) * 100).toFixed(2)) : null;
  const profitPctWhenSuggested = (displayProfitWhenSuggested !== null && displayedSalePrice !== null) ? Number(((displayProfitWhenSuggested / (displayedSalePrice + shippingPaid)) * 100).toFixed(2)) : null;

  // markup vs average (how far above/below market sell is compared to rolling average)
  const markupAmount = (displayedSalePrice !== null && rollingAvg !== null) ? Number((displayedSalePrice - rollingAvg).toFixed(2)) : null;
  const markupPct = (displayedSalePrice !== null && rollingAvg !== null && rollingAvg !== 0) ? Number(((markupAmount / rollingAvg) * 100).toFixed(2)) : null;
  ...
  // cache key helpers
  function cacheKeyForQuery(q) { return `est:${q}`; }
  ...
  async function fetchServerEstimate(q, force = false) {
    ...
    try {
      if (!force) {
        ...
        if (raw) {
          ...
          if (parsed && parsed.ts && (Date.now() - parsed.ts < (1000 * 60 * 5))) { // 5m cache
            // only update state if the cached value differs to avoid re-render loops
            try {
              const same = serverEstimate && JSON.stringify(serverEstimate) === JSON.stringify(parsed.val);
    ...
    try {
      const resp = await fetch('/api/estimate/from-actives', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ query: q, buyPrice: Number(buyPrice || 0) }) });
      if (!resp.ok) {
      ...
      try { sessionStorage.setItem(key, JSON.stringify({ ts: Date.now(), val: json })); } catch (e) {}
      setEstLoading(false);
      return json;
    } catch (e) {
  ...
  const itemQuery = itemToShow && (itemToShow.title || itemToShow.query) ? String(itemToShow.title || itemToShow.query) : '';
  React.useEffect(() => {
    let mounted = true;
    (async () => {
      try {
  ...
  const allVals = [].concat(series.avg.map(p=>p.v), series.min.map(p=>p.v), series.max.map(p=>p.v)).filter(v=>v!==null && v!==undefined);
  ...
  return (
    <main className="dr-analytics">
      ...
      <div className="dr-searchbar-wrapper">
        <SearchBar onSearch={() => {}} onOpenCamera={() => {}} onOpenImage={() => {}} />
      </div>

      <div className="dr-main-card">
        <img className="dr-thumb-large" src={itemToShow && itemToShow.thumbnail ? itemToShow.thumbnail : '/vite.svg'} alt="thumb" />
        <div style={{flex:1}}>
          <h3 className="dr-analytics-title" style={{margin:0}}>{itemToShow ? (itemToShow.title || itemToShow.query) : 'Item analytics'}</h3>
          <div style={{display:'flex',alignItems:'flex-end',gap:8,marginTop:12}}>
            <div className="dr-chart-svg" style={{height:260}}>
              <div className="dr-chart-card" style={{height:260}}>
                <AnalyticsChart series={series} width={680} height={260} showFill={false} showMinMax={true} accent={'var(--accent)'} primary={'var(--primary)'} showAxes={true} xTicks={6} yTicks={3} />
              </div>
            </div>
          </div>
        </div>
  ...
  <div style={{display:'grid',gridTemplateColumns:'1.4fr 360px',gap:16,alignItems:'start'}}>
        <div>
          <div className="dr-table">
            {top5.map((r,i) => (
              <div key={i} className="dr-table-row">
                <div className="left">
                  <a className="title" href="#" onClick={(e)=>{e.preventDefault(); setSelectedItem(recent[i]);}}>{r.title}</a>
                  <div className="tags">{r.platform ? <span className="chip">{r.platform.toUpperCase()}</span> : null} {r.year ? <span className="chip">{r.year}</span> : null}</div>
                </div>
                <div className="right">
        ...
        <div className="dr-right-panel">
          <div className="dr-kpi-grid">
            <MetricBox label="Rolling average" value={rollingAvg ? `$${rollingAvg}` : '—'} sub={rollingAvg ? 'Last 3 months' : ''} />
            <MetricBox label="Days to sell (est)" value={daysToSell ? `${daysToSell} days` : '—'} sub={'Estimate from trend'} />
            <div style={{position:'relative'}}>
              <MetricBox
                label={"Sell"}
                value={displayedSalePrice !== null ? `$${displayedSalePrice.toFixed(2)}` : '—'}
                sub={(
                  <>
                    Profit: {authoritativeBuy !== null ? (displayProfitWhenBought !== null ? `$${displayProfitWhenBought.toFixed(2)} (${profitPctWhenBought}% )` : '—') : (displayProfitWhenSuggested !== null ? `$${displayProfitWhenSuggested.toFixed(2)} (${profitPctWhenSuggested}% )` : '—')}
                    {requiredSalePrice !== null && authoritativeBuy !== null ? <div style={{fontSize:11,color:'var(--muted)'}}>Required sale for {profitPct}% margin: ${requiredSalePrice.toFixed(2)}</div> : null}
                  </>
                )}
              />
              <button title="Fees & assumptions" onClick={() => setShowInfo(s => !s)} style={{position:'absolute',right:6,top:6,border:'none',background:'transparent',cursor:'pointer'}}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="12" cy="12" r="10" /><path d="M12 8v.01" /><path d="M11 12h1v5h1" /></svg>
              </button>
              {showInfo ? (
                <div style={{position:'absolute',right:6,top:36,zIndex:50,minWidth:180,padding:8,background:'var(--card)',boxShadow:'0 6px 18px rgba(0,0,0,0.12)',borderRadius:6,fontSize:12}}>
                  <div style={{fontWeight:600,marginBottom:6}}>Fees & assumptions</div>
                  <div>Fee rate: {Math.round(feeRate*10000)/100}%</div>
                  <div>Shipping (paid/cost): ${shippingPaid}/${shippingEstimate}</div>
                </div>
              ) : null}
            </div>
            <MetricBox
              label="Suggested buy price"
              value={suggestedBuy !== null ? `$${suggestedBuy.toFixed(2)}` : '—'}
              sub={typeof sellPrice === 'number' ? 'Based on market sell price' : (netForSuggested !== null ? 'Based on active listings' : '')}
            />
          </div>

          <div className="dr-slider" style={{marginTop:8}}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
              <label style={{fontSize:12,color:'var(--muted)'}}>Profit margin: {profitPct}%</label>
              <MdOutlinePriceCheck size={20} title="Price simulator" style={{cursor:'pointer'}} onClick={() => { /* future: open modal */ }} />
            </div>
            <input type="range" min={0} max={100} value={profitPct} onChange={(e)=>{ setProfitPct(Number(e.target.value)); }} />

            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginTop:10,gap:8}}>
              <div style={{flex:1}}>
                <label style={{fontSize:12,color:'var(--muted)'}}>Bought price (removed)</label>
                <div style={{fontSize:12,color:'var(--muted)'}}>Bought price input has been removed from Analytics.</div>
              </div>
            </div>
          </div>
        </div>
