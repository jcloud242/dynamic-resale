...
export default function History({ onNavigateToAnalytics }) {
  ...
  function toggleTag(label) {
    ...
    if (!availableTags.includes(norm)) {
      const nextAvailable = [norm, ...availableTags.filter(t => t !== norm)].slice(0,15);
      setAvailableTags(nextAvailable);
      try { localStorage.setItem('dr_tags', JSON.stringify(nextAvailable)); } catch (e) {}
    }
  ...
  try {
    ...
    for (const s of [...seed, ...(availableTags || [])]) {
      ...
      combinedKeys[k] = true;
      combined.push({ raw: s, norm: k, display: seed.includes(s) ? s : (String(s).split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')), active: (activeTags || []).includes(k) });
    }
  } catch (e) {
    // fallback
  }
  ...
  return (
    <main className="dr-history">
      {/* header intentionally removed: SearchBar shows context */}

      <div style={{display:'flex',gap:8,alignItems:'center',marginBottom:8}}>
        <div style={{flex:1}}>
          {/* reuse SearchBar UI but hide scan/photo icons on history */}
          <SearchBar onSearch={(p) => { const q = (typeof p === 'object') ? (p.query || '') : p; searchLocal(q); }} onOpenCamera={() => {}} onOpenImage={() => {}} showScans={false} placeholder="Search History" />
        </div>
        {/* right side: filter icon only (SearchBar has its own submit button) */}
        <button ref={filterBtnRef} className="dr-filter-btn dr-icon-btn" aria-label="Sort/Filter" onClick={(e) => {
          const rect = filterBtnRef.current && filterBtnRef.current.getBoundingClientRect();
          setOverlayPos(rect ? { top: rect.bottom + window.scrollY + 8, left: rect.left + window.scrollX } : null);
          setSortOverlayOpen(s => !s);
        }} title="Filter">
          <span className="dr-icon-inner"><IoFilter size={20} /></span>
        </button>
      </div>
  ...
  <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center',marginBottom:12}}>
        {combined.map((tObj, i) => {
          ...
          return (
            <div key={i} className={`dr-history-tag ${active ? 'active' : ''}`}>
              <button className="dr-history-tag-btn" onClick={() => toggleTag(tObj.raw || tObj.norm)}>
                <span className="dr-history-tag-label">{display}</span>
                {active && <span className="dr-history-tag-x" onClick={(e) => { e.stopPropagation(); toggleTag(tObj.raw || tObj.norm); }} aria-label={`Remove ${display}`}>×</span>}
              </button>
            </div>
          );
        })}
      ...
      {sortOverlayOpen && (
        <div className="dr-overlay" onClick={() => setSortOverlayOpen(false)}>
          <div className="dr-overlay-panel" style={overlayPos ? { position: 'fixed', left: overlayPos.left + 'px', top: overlayPos.top + 'px' } : {}} onClick={(e)=>e.stopPropagation()}>
            <h4>Filter by</h4>
            <div style={{display:'flex',flexDirection:'column',gap:8}}>
              <button onClick={() => { setSortMode('title'); setSortOverlayOpen(false); searchLocal(query); }}>Title</button>
              <button onClick={() => { setSortMode('category'); setSortOverlayOpen(false); searchLocal(query); }}>Category</button>
              <button onClick={() => { setSortMode('value'); setSortOverlayOpen(false); searchLocal(query); }}>Value</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div ref={recentRef} className="dr-recent-wrapper" style={{marginTop:8}}>
        <ResultList items={pageItems} active={false} hideChart={true} onAnalyticsClick={(it) => { try { if (onNavigateToAnalytics) onNavigateToAnalytics(it); } catch (e) {} }} />
      </div>

      {/* Pagination */}
      <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:8,marginTop:12}}>
        <div className="dr-pagination">
          <button aria-label="Previous" onClick={() => setPage(p => Math.max(1, p-1))} className="chev">❮</button>
          {Array.from({length: Math.min(totalPages, 7)}, (_, i) => i + 1).map(n => (
            <button key={n} className={`page-num ${n === page ? 'active' : ''}`} onClick={() => setPage(n)}>{n}</button>
          ))}
          <button aria-label="Next" onClick={() => setPage(p => Math.min(totalPages, p+1))} className="chev">❯</button>
        </div>
      </div>
    </main>
  );
...
function TagInput({ onAdd }) {
  ...
  return (
    <div className="dr-tag-adder">
      <button className="dr-tag-plus" onClick={() => setOpen(o => !o)} aria-label="Add tag">
        <FaRegSquarePlus size={20} />
      </button>
      {open && (
        <div style={{display:'flex',alignItems:'center'}}>
          <input autoFocus placeholder="Add new tag" value={val} onChange={(e)=>setVal(e.target.value)} onKeyDown={(e)=>{ if (e.key === 'Enter') { if (val && val.trim()) { onAdd(val.trim()); setVal(''); setOpen(false); } } }} />
        </div>
      )}
    </div>
  );
